...
//このスクリプトはCubeにはる
public class RotateCube41_Orange : MonoBehaviour {

	GameObject Parent; //親オブジェクト
	Vector3 current; //現在の回転値
	Vector3 destination; //目的となる回転値
	public float speed = 10; //回転のスピード
	...
	public FlagManeger flagManeger; //FlagManegerを使うため
	...
	void Update () {
		...
		if (isFall) {
			...
			if (time == 0) {
				...
				if (Physics.Raycast (rayY, out hit, 0.8f)) {
